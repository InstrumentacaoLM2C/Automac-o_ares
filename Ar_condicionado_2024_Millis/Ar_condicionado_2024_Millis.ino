...
const unsigned long interval2 = 21600000; // Intervalo de automação dos ar
float temperaturaF;
int DezMin =0;
...
void loop() {
  ...
  if (Serial.available() > 0) // Se há dados disponíveis na porta serial
    {
        d1 = Serial.read(); // Lê o comando recebido
        bool newData = true; // Indica que há um novo comando disponível
 
        if (newData == true) // Se há um novo comando
        {
            switch (d1) // Verifica qual comando foi recebido
            {
               case 'L':
               sendCode(1,1);
               d1 = 'o'; //significa nada
